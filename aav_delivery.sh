...
#Paired end pipeline not developed as the amfar set was run on single end kit
#commenting out in case this is needed in the future but will need to edit it--currently based on hsv mutagenesis script
##  PAIRED-END  ##
if [[ $paired == "true" ]]
then
echo "Paired-end not yet supported for this pipeline"
...
printf "\n\nAdapter trimming ... \n\n\n"
mkdir -p ./trimmed_fastq
bbduk.sh in=$in_fastq out='./trimmed_fastq/'$sampname'_trimmed_tmp.fastq.gz' ref=adapters k=21 ktrim=r mink=4 hdist=2 overwrite=TRUE t=$SLURM_CPUS_PER_TASK 
bbduk.sh in='./trimmed_fastq/'$sampname'_trimmed_tmp.fastq.gz'  out='./trimmed_fastq/'$sampname'_trimmed.fastq.gz' ref=adapters k=21 ktrim=l mink=4 hdist=2 overwrite=TRUE t=$SLURM_CPUS_PER_TASK 
...
# samtools view -bh -q 10 -o './mapped_reads/'$sampname'_'$ref'.bam' -T ./refs/$ref'.fasta' './mapped_reads/'$sampname'_'$ref'.sam'  
